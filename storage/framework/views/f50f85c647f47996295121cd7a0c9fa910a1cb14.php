...
<div id="webuploader" class="webuploader">
    <input class="img-id" type="hidden" name="storage_id" value="<?php echo e($storage->id ?? 0); ?>">
    <div class="img-preview">
        ...
        <img style="display:none;" src="<?php echo e($storage->path ?? ''); ?>">
    </div>
</div>
<!-- 封面 -->
...
<script type="text/javascript">
        ...
        uploader.md5File(file).then(function(val) {
            $.get("<?php echo e(route('Admin.storages.picture')); ?>", {md5: val}, function(response) {
                if (response.code) {
                    ...
                    $("#webuploader .img-id").val(response.data.id);
    ...
    uploader.on("uploadSuccess", function(file, response) {
        if (response.code == 1) {
            ...
            $("#webuploader .img-id").val("");
            $("#webuploader .img-id").val(response.id);
            $("#webuploader .fa-close").show();
        } else {
