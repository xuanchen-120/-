...
<form action="{{ route('chant.edit',$chant) }}" method="POST" accept-charset="utf-8">
    <div class="login_container" >
        <div class="login_block login_margin">
            <i class="icon icon-file-word login_i" style="font-size:1rem"></i>
            <input type="tel" name="number" value="{{ $chant->number }}" placeholder="请输入诵经次数" class="input login_input" >
        </div>
        @csrf
        @method('post')
